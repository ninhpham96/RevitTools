    ...
    public class Class1 : IExternalCommand
    {
        ...
        public Duct CreateDuctBetweenConnectors(UIDocument uiDocument)
        {
                    ...
                    if (family.FamilyCategory.Name != "Duct Accessories")
                    {
                        ...
                        while (csi.MoveNext())
                        {
                            if (null == connector2)
                            {
                                ...
                                if (connector1 != null && conn != null && Math.Abs(conn.Origin.Y - connector1.Origin.Y) < 0.001)
                                {
                                    connector2 = conn;
                                    break;
                                }
            ...
            if (null != connector1 && null != connector2 && levelId != ElementId.InvalidElementId)
            {
                ...
                DuctType ductType = collector.OfCategory(BuiltInCategory.OST_DuctFitting).Cast<DuctType>().First();
